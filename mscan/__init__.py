...
def getUsage(user_ID):
	if request.method == "POST":
		...
		if user:
			...
			stats["Number of Calls to Fix in CH"] = callsFixedCH(user_ID).get('number')
			stats["Seconds of Calls to Fix in CH"] = callsFixedCH(user_ID).get('duration')
			...
			stats["Abroad Country 1"]  = callsToAbroadLandX(user_ID, 0).get('country')
