...
def getUsage(user_ID):
	if request.method == "POST":
		...
		if user:
			...
			stats["Days in most visited country"] = getDaysInMostVisitedCountry(userID)
			stats["Calls from abroad to CH"] = callsToCHfromAbroad(userID)
			stats["N.1 frequent number"] = get3MostFrequentlyCalledNumbers(userID).get('number1')
			stats["N.2 frequent number"] = get3MostFrequentlyCalledNumbers(userID).get('number2')
			stats["N.3 frequent number"] = get3MostFrequentlyCalledNumbers(userID).get('number3')
			stats["SMS while roaming "] = getSMSWhileRoaming(userID)
