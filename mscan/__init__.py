...
def getUsage(user_ID):
	if request.method == "POST":
		...
		if user:
			...
			stats["Number of Calls to landlines in CH"] = callsFixedCH(user_ID).get('number')
			stats["Seconds of Calls to landlines in CH"] = callsFixedCH(user_ID).get('duration')
			stats["Number of Calls to Mobile in CH"] = CallsMobileCH(user_ID).get('number')
			stats["Seconds of Calls to Mobile in CH"] = CallsMobileCH(user_ID).get('duration')
			stats["Sms from CH and to CH"] = SMS_toCH(user_ID)
			stats["Data in CH"] = dataCH(user_ID)  + " MBytes"
			stats["Sms to abroad"] = SMS_toABROAD(user_ID)
			stats["#1 foreign country called"]  = callsToAbroadLandX(user_ID, 0).get('country')
			stats["Number Calls to Abroad 1"]  = callsToAbroadLandX(user_ID, 0).get('number')
			stats["Seconds Calls to Abroad 1"]  = callsToAbroadLandX(user_ID, 0).get('duration')
