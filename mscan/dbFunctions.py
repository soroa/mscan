	...
	if user:
		...
		for c in callsToAbroad: 
			if c.call_number[:len(prefix)] == prefix:
				counter+= 1
				duration += int(c.duration)
	return {'number': str(int(mapTo30Days((float(counter)/2.0),getDaysSinceSignUp(user_ID)))), 'duration': str(int(mapTo30Days(float((float(duration)/120.0)),getDaysSinceSignUp(user_ID)))), 'country': xMostfrequentCountry}
